export const compressContent = {
  ...
  howItWorks: {
    ...
    body: [
      ...
      "One strategy is a Ghostscript pass with duplicate-image detection and font subsetting, used where that engine is available on the machine handling your file. The one that runs on every request is a lossless structural rewrite: unused objects are garbage-collected, content streams are sanitised, and everything is deflated. This one is safe on any document and typically accounts for the reduction you see on pure text files. The third re-encodes embedded images, downsampling anything longer than the preset's pixel ceiling and re-compressing it as progressive JPEG — and it only substitutes an image when the new version is genuinely smaller. The fourth applies only to documents that are already essentially scans: each page is rasterised whole. It destroys the text layer, so we only consider it when there is no meaningful text layer to lose.",
    ...
    specs: [
      { label: "Processing pipeline", value: "Lossless rewrite, image re-encode, full rasterise, plus a Ghostscript pass where available — smallest result wins" },
